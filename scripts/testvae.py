...
def main(opt):
    ...
    with torch.no_grad(), \
        ...
        model.ema_scope():
            ...
            #                                          x_T=start_code)
            for i in range(9):
                img_path = "/home/v-yanhwang/code/videostablediffusion/outputs/txt2img-samples/samples/0000{}.png".format(i)

                print(img_path)
                img = Image.open(img_path)

                img_size = (768, 768)  # set the desired image size
                img = resize(img, img_size)
                img_tensor = to_tensor(img)
                img_tensor = normalize(img_tensor, mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])  # normalize image tensor
                with torch.no_grad():
                    input=img_tensor.unsqueeze(0).half().cuda()
                    reconstructions, posterior = model.first_stage_model(input)
                
                # x_samples = model.decode_first_stage(samples)
                x_samples = torch.clamp((reconstructions + 1.0) / 2.0, min=0.0, max=1.0)

                for x_sample in x_samples:
                    x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                    img = Image.fromarray(x_sample.astype(np.uint8))
                    # img = put_watermark(img, wm_encoder)
                    img.save(os.path.join(sample_path, f"{base_count:06}.png"))
                    base_count += 1
                    sample_count += 1
