...
def main(opt):
    ...
    start_code = None
    if opt.fixed_code:
        start_code = torch.randn([opt.n_samples, opt.C, opt.T, opt.H // opt.f, opt.W // opt.f], device=device)
        start_code = torch.randn([opt.n_samples, opt.C, opt.H // opt.f, opt.W // opt.f], device=device)
        start_code = repeat(start_code,"b c h w -> b c t h w",t=opt.T)
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                for prompts in tqdm(data, desc="data"):
                    ...
                    if isinstance(prompts, tuple):
                        prompts = list(prompts)
                    c = model.get_learned_conditioning(prompts)
                    shape = [opt.C, opt.T, opt.H // opt.f, opt.W // opt.f]
                    samples, _ = sampler.sample(S=opt.steps,
                                                     ...
                                                     eta=opt.ddim_eta,
                                                     x_T=start_code)
                    print('shaples.shape',samples.shape)
